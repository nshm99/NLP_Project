...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = \
            nn.Parameter(torch.zeros((self.hidden_size),requires_grad=True))
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = torch.nn.Dropout(self.dropout_prob)

        self.hidden_to_logits_weight = \
            nn.Parameter(torch.zeros((self.hidden_size,self.n_classes),requires_grad=True))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)

        self.hidden_to_logits_bias = \
            nn.Parameter(torch.zeros((self.n_classes),requires_grad=True))
    ...
    def embedding_lookup(self, w):
        ...
        #!!
        selected = torch.index_select(self.embeddings,0,torch.flatten(w))
        x = selected.view(w.size(0),self.n_features*self.embed_size)
    ...
    def forward(self, w):
        ...
        relu_input = (x@self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        h = F.relu(relu_input)
        drop_out_h = self.dropout(h)

        logits = (drop_out_h@self.hidden_to_logits_weight) + self.hidden_to_logits_bias
