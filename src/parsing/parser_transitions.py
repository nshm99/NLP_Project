...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition == 'S'):
            poped_word = self.buffer.pop(0)
            self.stack.append(poped_word)
        elif(transition == 'LA'):
            stack_top = self.stack[-1]
            poped_dependant = self.stack.pop(-2)
            self.dependencies.append((stack_top,poped_dependant))
        elif(transition == 'RA'):
            stack_top = self.stack[-2]
            poped_dependant = self.stack.pop(-1)
            self.dependencies.append((stack_top,poped_dependant))
